...
# myrakuOS targeted KDE payload with host virtualization support
...
# Remove only packages that are explicitly known to be unwanted. Missing
# packages are ignored so the list remains valid across Fedora point releases.
dnf5.real -y remove --setopt=clean_requirements_on_remove=False \
  --skip-unavailable \
  gamemode mangohud mangohud.i686 goverlay lutris heroic-games-launcher \
  protonplus steam \
  ...
  libva-intel-media-driver firefox firefox-langpacks \
  glibc-all-langpacks \
